...
opt router 10.10.0.1 # The Pi's IP address on wlan0 which we will set up shortly.
opt lease 864000 # 10 day DHCP lease time in seconds 
EOF

 sed -i 's/DHCPD_ENABLED\=\"no\"/#DHCP_ENABLED\=\"no\"/g' /etc/default/udhcpd

 sed -i 's/INTERFACES\=\"AUTO\"/DHCP_ENABLED\=\"\"/g' /etc/default/ifplugd
