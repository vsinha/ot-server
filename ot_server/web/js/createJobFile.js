...
var createPipetteGroup = {
  ...
  'transfer' : function (theDeck, theTool, transferArray) {
    ...
    for(var i=0;i<transferArray.length;i++) {
      ...
      if(thisTransferParams.blowout) {
        _addMovements({'plunger':'blowout'});
      }
      _addMovements({'plunger' : 'resting'});
    }
  ...
  'distribute' : function (theDeck, theTool, distributeGroup) {
    ...
    for(var i=0;i<toParamsArray.length;i++) {
      totalVolume += toParamsArray[i].volume;
    }

    // always add 10% on distribute, to compensate for the curve when dispensing
  ...
  'mix' : function (theDeck, theTool, mixArray) {
    ...
    for(var i=0;i<mixArray.length;i++) {
      mixArray[i].volume *= -1; // this is so we accomodate for the moving liquid surface height
      var mixMoveCommands = makePipettingMotion(theDeck, theTool, mixArray[i], true);
...
function makePipettingMotion (theDeck, theTool, thisParams, shouldDropPlunger) {
  ...
  if(theDeck[containerName] && theDeck[containerName].locations) {
    ...
    if(shouldDropPlunger) {
      for(var q=0;q<3;q++) {
        theTool['current-plunger'] = .8;
        moveArray.push({
          'plunger' : theTool['current-plunger']
        });
        theTool['current-plunger'] = .9;
        moveArray.push({
          'plunger' : theTool['current-plunger']
        });
      }
    ...
    if(thisParams.repetitions) {
      ...
      for(var i=0;i<thisParams.repetitions;i++) {
        ...
        theTool['current-plunger'] += (plungerPercentage*-1);
        moveArray.push({
          'plunger' : theTool['current-plunger']
        });
      }
    ...
    // if it's not a mix command, then just pipette it
      ...
      console.log('going from '+theTool['current-plunger']);
      ...
      console.log('to '+theTool['current-plunger']);
      console.log('with extra % at '+extraPercentage);
...
function getPercentage (thisVolume, theTool) {
  ...
  console.log('\n\n***');
  console.log('read: '+realVolume);
  console.log('mapped: '+absVolume);
  console.log('old percentage: '+(realVolume/theTool.volume));
  console.log('new percentage: '+(absVolume/theTool.volume));
